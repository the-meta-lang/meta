...
for TEST in $TESTS; do
		...
		mkdir -p $TMP_DIR
		# Run the test
		# First we need to grab the mlisp file
		MLISP_FILE=$(find "$TEST" -name "*.mlisp")
		MLISP_NAME=$(basename "$MLISP_FILE")
		cp "$MLISP_FILE" "$TMP_DIR/$MLISP_NAME"
		# Then we need to compile the code file
		$COMPILER_BIN "$TMP_DIR/$MLISP_NAME" > "$TMP_DIR/code.asm"
		# And compile the generated assembly
		FILE="$TMP_DIR/code.asm"

		nasm -F dwarf -g -f elf32 -o $FILE.o $FILE 
		ld -m elf_i386 -o "$TMP_DIR/code.bin" $FILE.o
		rm $FILE.o

		OUTPUT=$("$TMP_DIR/code.bin")
